...
function dotenvConfig(options: { path: string }) {
...
describe('Database Constraints Test Suite (DATA-06)', () => {
  ...
  beforeAll(async () => {
    ...
    // Get seeded user, facility, slot, vehicle for testing
    const userRes = await client.query(
      `SELECT "id" FROM "users" WHERE "phone" = '+923459998887' LIMIT 1;`,
    );
    userId = userRes.rows[0].id;

    const facilityRes = await client.query(
      `SELECT "id" FROM "facilities" WHERE "name" = 'Demo Plaza' LIMIT 1;`,
    );
    facilityId = facilityRes.rows[0].id;

    const slotRes = await client.query(
      `SELECT "id" FROM "slots" WHERE "facility_id" = $1 AND "slot_code" = 'AB-1' LIMIT 1;`,
      [facilityId],
    );
    ...
    // Clean up any existing test bookings on slot AB-1
    await client.query(`DELETE FROM "bookings" WHERE "slot_id" = $1;`, [
      slotId,
    ]);
  });
  ...
  afterAll(async () => {
    if (client) {
      // Clean up test bookings and vehicle
      await client.query(`DELETE FROM "bookings" WHERE "slot_id" = $1;`, [
        slotId,
      ]);
  ...
  it('BOOK-04 & DATA-06: Overlapping booking MUST be rejected by database exclusion constraint (bookings_no_overlap)', async () => {
    // Attempt booking from 10:30 to 11:30 (overlaps with 10:00-11:00 and 11:00-12:00)
    ...
    // Exclusion constraint violation in Postgres has code 23P01 or constraint name 'bookings_no_overlap'
